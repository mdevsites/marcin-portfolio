...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      {/* GŁÓWNY GRID */}
      <div className="relative max-w-7xl mx-auto px-6 flex flex-col-reverse md:flex-row items-center md:justify-between h-full z-10 gap-10 md:gap-0 mt-10 md:mt-0">
        ...
        <div className="text-center md:text-left md:max-w-xl space-y-6">
          ...
          <div className="mt-10 flex flex-wrap justify-center md:justify-start gap-6">
            {(showIcons ? icons : stats).map((item, idx) => (
              <div
                key={idx}
                className="relative w-28 md:w-36 py-6 px-3 flex flex-col items-center justify-center text-center"
              >
                <div
        ...
        <div className="mb-6 md:mb-0 md:ml-12 md:flex-1 flex justify-center relative">
          <img
            ...
            className="w-full max-w-[240px] md:max-w-md rounded-lg shadow-xl transform transition-transform hover:-translate-y-1"
          />
        </div>
      </div>
      ...
      <div className="mt-12 flex justify-center z-20 px-4">
        <div className="relative bg-transparent px-8 py-6 rounded-2xl text-white text-center flex items-center max-w-3xl w-full mx-auto overflow-hidden">

          <button
            onClick={prevOpinion}
            className="absolute left-1 top-1/2 -translate-y-1/2 p-2 rounded-full hover:bg-white/10 transition"
          >
          ...
          <div className="mx-auto text-center flex-1">
            <p
              className={`italic text-lg md:text-xl px-6 py-4 rounded-2xl bg-white/10 backdrop-blur-md shadow-md inline-block transition-all duration-700 ease-out ${
                showOpinion ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-6"
              }`}
            >
              “{opinions[currentOpinion].text}”
            </p>
            <p className="mt-2 font-semibold text-white">{opinions[currentOpinion].author}</p>
          </div>

          <button
            onClick={nextOpinion}
            className="absolute right-1 top-1/2 -translate-y-1/2 p-2 rounded-full hover:bg-white/10 transition"
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor">
      ...
      {/* STRZAŁKA */}
      <div
        className="absolute bottom-0 left-1/2 -translate-x-1/2 mb-4 cursor-pointer group"
        onClick={() => {
      ...
      >
        ...
        <svg className="w-7 h-7 text-white animate-bounce opacity-80 group-hover:opacity-100 transition" fill="none" stroke="currentColor">
